...
export default function BehaviorDevelopment() {
  ...
  // "N번과 " 패턴으로 시작하는 기록 = 관련 기록 (백엔드가 자동 생성)
  // 예: "2번과 테스트" → 원본은 2번 학생의 기록
  const parseRelatedPrefix = (content) => {
    const m = content?.match(/^(\d+)번과\s/);
    if (!m) return null;
    const num = parseInt(m[1]);
    const origin = students.find(s => s.student_number === num);
    return origin ? { studentNumber: num, student: origin, cleanContent: content.replace(/^\d+번과\s/, '') } : null;
  };
                      ...
                      <div key={log.id} style={{
                        ...
                        <div style={{flex:1}}>
                          {!related && editingLog===log.id ? (
                            ...
                            <div>
                              {related && (
                                <button onClick={()=>setSelectedStudent(related.student)}
                                  ...
                                  ↗ {related.studentNumber}번 {related.student.name}의 기록
                                </button>
                              )}
                              <span style={{fontSize:'14px',color: related ? 'var(--text-secondary)' : 'var(--text-primary)', display:'block'}}>
                                {related ? related.cleanContent : log.content}
                              </span>
                              {!related && log.related_student_ids?.length>0&&(
                        ...
                        {!related ? (
                          ...
                          <span style={{fontSize:'10px',color:'#94a3b8',whiteSpace:'nowrap'}}>읽기전용</span>
                        )}
                      </div>
                      );
                    })
